...
namespace SP_Gen
{
    public partial class frmMain : Form
    {
        ...
        private void GenerateSQL(string DatabaseName, string tableName, DataRow[] rows, DataRow[] rowsFooter)
        {
            ...
            {
                txtSQL.Document.Text = SQL_Generator.CreateSelectProperties(Prefix + tableName + "_SelectAll", tableName, rows);
                txtSQL.Document.Text += "\r\n\r\n";
            }

            if (GenerateSelectRow)
            {
                txtSQL.Document.Text += SQL_Generator.CreateSelectListSP(Prefix + tableName + "_SelectRow", tableName, rows);
                txtSQL.Document.Text += "\r\n\r\n";
            }
            ...
            {
                txtSQL.Document.Text += SQL_Generator.CreateSaveXmlSP(txtfootertable.Text , tableName, rows,rowsFooter);
                txtSQL.Document.Text += "\r\n\r\n";
            }
        ...
        private void tbtnGenerate_Click(object sender, EventArgs e)
        {
            if (tvEntities.SelectedNode != null && tvEntities.SelectedNode.Level == 1)
            {
                if (tvEntities.SelectedNode != null)
                {
                    ...
                    DataRow[] rowsFooter = null;
                    if (txtfootertable.Text != "")
                    {
                        rowsFooter = dsTablesAndColumns.Tables[0].Select("Table_Name = '" + txtfootertable.Text + "'");
                    
                    }

                    GenerateSQL(dbName, tableName, rows,rowsFooter);
        ...
        private void GeneratexmlSP_Click(object sender, EventArgs e)
        {
              string tableName = tvEntities.SelectedNode.Text;
              if (txtfootertable.Text != "" && tableName != "")
              {
                 // txtSQL.Document.Text += SQL_Generator.CreateSaveXmlSP(txtfootertable, tableName);
                  txtSQL.Document.Text += "\r\n\r\n";
              }
              else
              {
                  MessageBox.Show("please choose the matser table and enter the footer table");
